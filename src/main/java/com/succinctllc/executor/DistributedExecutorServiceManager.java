...
public class DistributedExecutorServiceManager {
    ...
    //TODO: implements this later as a possible optimization for when nodes go down
    //private final MultiMap<Member, HazelcastWork>                 memberWork;
    private final ExecutorService                           workDistributor;
    
    //a member must indicate that it is ready to receive work in order for work to be given to it
    private final CopyOnWriteArrayList<Member>                                    readyMembers = new CopyOnWriteArrayList<Member>();
    ...
    protected DistributedExecutorServiceManager(InternalBuilderStep2 internalBuilderStep1) {
        ...
        //map.addIndex(attribute, ordered) add index for timestamp so we can query for old items
        
        //add long TTL for the work map to expire elements if they are not touched after so long...
        ...
        if(serviceManagersByTopology.putIfAbsent(topologyName, this) != null) {
            throw new IllegalArgumentException("A DistributedExecutorServiceManager already exists for the topology "+topologyName);
        }
    ...
    private void checkReadyMembers() {
        //MultiTask<Boolean> task = new MultiTask<Boolean>(new IsReady(topologyName), this.hazelcast.getCluster().getMembers());
        Collection<MemberResponse<Boolean>> results = MemberTasks.executeOptimistic(
             this.communicationExecutorService, 
             this.hazelcast.getCluster().getMembers(), 
        ...
        for(MemberResponse<Boolean> result : results) {
            if(result.getValue()) {
                ...
                if(m.equals(thisMember))
                    m = thisMember;
                this.readyMembers.addIfAbsent(m);
            }
        }
    }
    ...
    /**
     * returns the list of members that are online and have indicated they are ready to recieve work
     * @return
     */
    protected List<Member> getAvailableMembers() {
        //FIXME: put this in another thread with an exponential backoff
        //this is temporary
        if((System.currentTimeMillis() - lastChecked) >= 10000) { //check every 10 seconds for ready members
    ...
    public static DistributedExecutorServiceManager getDistributedExecutorServiceManager(String topology){
